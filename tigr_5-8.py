...
def select_balanced_tasks(tasks, total=40):
    fem = []
    masc = []
    plural = []

    for t in tasks:
        text = t.get("stimulus_text", "").lower() if isinstance(t, dict) else t.lower()

        # простая эвристика по окончанию / смыслу
        if "они" in text.lower() or "рабочие" in text.lower() or "дети" in text.lower():
            plural.append(t)
        elif "она" in text.lower() or "мама" in text.lower() or "девочка" in text.lower() or "бабушка" in text.lower():
            fem.append(t)
        else:
            masc.append(t)

    # гарантируем минимум 13
    selected = []
    selected += random.sample(fem, min(13, len(fem)))
    selected += random.sample(masc, min(13, len(masc)))
    selected += random.sample(plural, min(13, len(plural)))
...
if st.session_state.current_step == 1:  # Инструкция
    ...
    if st.button("Начать тренировку"):
        ...
        if "task5_selected" not in st.session_state:
            st.session_state.task5_selected = select_balanced_tasks(task_data.gender_easy, 40)
    ...
    answ_co = len(st.session_state.task5_selected)
    if index < answ_co:
        st.header("Задание 4.1")
        st.write("Выберите правильный глагол с опорой на предложение-образец")
        task5 = st.session_state.task5_selected[index]
...
############################################################################################################################################
if "task7_selected" not in st.session_state:
    st.session_state.task7_selected = random.sample(
        task_data.gender_middle_plus,
        min(40, len(task_data.gender_middle_plus))
    )
    st.session_state.task7_options = random.sample(
        task_data.gender_middle_plus_opt,
        min(40, len(task_data.gender_middle_plus_opt))
    )
...
if st.session_state.current_step == 7:
    ...
    if index < len(task_data.gender_middle_plus_test):
        ...
        html = create_task7_html(st.session_state.task7_selected[index])
    ...
    total_questions = len(st.session_state.task7_selected)
    ...
    if index < total_questions:
        ...
        # Варианты ответов
        choice = None
        for answer in st.session_state.task7_options[index]:
            if st.button(answer, key=f"q7_{index}_{answer}"):
                choice = answer
        
        if choice is not None:
            # Сохраняем ответ
            st.session_state.responses[f"Задание 7: {st.session_state.task7_selected[index]}"] = choice
            st.rerun()
        ...
        if st.button("Перейти к следующему заданию"):
            ...
            st.rerun()
############################################################################################################################################
if "task8_selected" not in st.session_state:
    st.session_state.task8_selected = select_balanced_tasks(
        task_data.gender_complex,
        min(40, len(task_data.gender_complex))
    )
...
if st.session_state.current_step == 10:
    ...
    answ_co = len(st.session_state.task8_selected)
    if index < answ_co:
        st.header("Задание 4.4")
        # Создаем HTML с новым дизайном
        html = create_task8_html(st.session_state.task8_selected[index])
        st.components.v1.html(html, height=150)
        ...
        if st.button("Далее") and answer:
            st.session_state.responses[f"Задание 8: {st.session_state.task8_selected[index]}"] = answer
            st.rerun()
